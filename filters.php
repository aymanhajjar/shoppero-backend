...
$colors_name->bind-param('s',$color_id);
$product_id=$_POST("id");
$product_price=$mysqli->prepare{"select price from products WHERE id=?"};
$product_price->bind-param('s',$product_id);
$material_id=$_POST("id");
$material_name=$mysqli->prepare{"select material_name from materials WHERE id=?"};
$material_name->bind-param('s',$material_id);
$brand_id=$_POST("id");
$brand_name=$mysqli->prepare{"select brand_naem from brands WHERE id=?"};
$brand_name->bind-param('s',$brand_id);
